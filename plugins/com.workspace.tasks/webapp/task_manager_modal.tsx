...
import {
  Plus, Search, MoreVertical, Calendar, CheckCircle2,
  Circle, Clock, Trash2, Filter, ArrowUpDown,
...
export interface Task {
  ...
  dueDate?: string; createdAt: string; assignee?: string;
}

const NAV_ITEMS = [
  { id: 'all', label: 'All Tasks', icon: ListIcon, showCount: true },
...
const MOCK_TASKS: Task[] = [
  {
    id: 'TSK-101', title: 'Review design guidelines',
    description: 'Ensure compliance with DESIGN_TASTE.md for the new task manager component.',
    status: 'completed', priority: 'high', dueDate: '2024-06-20', createdAt: '2024-06-19', assignee: 'Jules'
  },
  {
    id: 'TSK-102', title: 'Implement RTL support',
    description: 'Apply logical CSS properties to handle bidirectional layouts seamlessly.',
    status: 'in-progress', priority: 'high', dueDate: '2024-06-21', createdAt: '2024-06-19', assignee: 'Jules'
  },
  {
    id: 'TSK-103', title: 'Refactor Task List view',
    description: 'Migrate to a high-density, professional grid layout with refined typography.',
    status: 'todo', priority: 'medium', dueDate: '2024-06-22', createdAt: '2024-06-19', assignee: 'Jules'
  },
  {
    id: 'TSK-104', title: 'Finalize SCSS tokens',
    description: 'Sync Zinc-based neutral palette across all component states.',
    status: 'todo', priority: 'low', dueDate: '2024-06-23', createdAt: '2024-06-19', assignee: 'Jules'
  }
];
...
const PriorityIndicator: React.FC<{ priority: TaskPriority }> = ({ priority }) => {
  ...
  const colors = { low: 'text-zinc-400', medium: 'text-amber-500', high: 'text-rose-500' };

  return (
    <div className="flex gap-0.5">
      {[40, 60, 100].map((op, i) => {
        const active = (i === 0) || (i === 1 && isMed) || (i === 2 && isHigh);
        return (
          <div
            key={i}
            className={`w-1 h-3 rounded-full ${active ? colors[priority] : 'bg-zinc-200'} bg-current`}
            style={{ opacity: active ? op / 100 : 1 }}
          />
        );
      })}
    </div>
      ...
      <div className="tm-modal" role="dialog" aria-modal="true">
        <aside className="tm-sidebar">
          <div className="tm-logo-area">
            <div className="tm-logo-mark"><Hash size={18} /></div>
          ...
          <nav className="tm-nav">
            {NAV_ITEMS.map(item => (
              <button key={item.id} className={`tm-nav-item ${item.id === 'all' ? 'is-active' : ''}`}>
                ...
                {item.showCount && <span className="tm-nav-count">{tasks.length}</span>}
              </button>
            ))}
          </nav>
        ...
        <main className="tm-main">
              ...
              <div className="tm-view-toggle">
                {[ {id:'list', icon:ListIcon}, {id:'grid', icon:LayoutGrid} ].map(v => (
                  <button key={v.id} onClick={() => setViewMode(v.id as any)}
                    className={`tm-toggle-btn ${viewMode === v.id ? 'is-active' : ''}`}><v.icon size={14} /></button>
                ))}
              </div>
              <button className="tm-btn-primary"><Plus size={16} /><span>Issue</span></button>
            </div>
          </header>
          ...
          <div className="tm-toolbar">
            <div className="tm-search">
              ...
              <input type="text" placeholder="Filter issues..." value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)} className="tm-search-input" />
            </div>
            <div className="flex items-center gap-2">
              {['Status', 'Priority'].map(label => (
                <button key={label} className="tm-toolbar-btn">
                  {label === 'Status' ? <Filter size={14} /> : <ArrowUpDown size={14} />}
                  <span>{label}</span>
                </button>
              ))}
            </div>
          ...
          <div className="tm-content">
            <div className="tm-list-header">
              {['Status', 'ID', 'Title', 'Priority', 'Due Date'].map(h => (
                <div key={h} className={`tm-col-${h.toLowerCase().replace(' ', '')}`}>{h}</div>
              ))}
            </div>
              ...
              {filteredTasks.map(task => (
                <div key={task.id} className={`tm-row ${task.status === 'completed' ? 'is-completed' : ''}`}>
                  <div className="tm-col-status">
                    <button onClick={() => toggleStatus(task.id)} className="tm-status-indicator">
                      {task.status === 'completed' ? <CheckCircle2 size={16} className="text-blue-600" /> :
                  ...
                  <div className="tm-col-priority"><PriorityIndicator priority={task.priority} /></div>
                  <div className="tm-col-date">{task.dueDate ? <div className="flex items-center gap-1.5 text-zinc-500"><Calendar size={12} /><span>{task.dueDate}</span></div> : '-'}</div>
                </div>
              ))}
              {filteredTasks.length === 0 && (
                <div className="tm-empty">
                  <div className="tm-empty-icon"><Search size={32} /></div>
